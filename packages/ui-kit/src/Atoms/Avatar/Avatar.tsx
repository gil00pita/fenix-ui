...
const InternalAvatar: React.ForwardRefRenderFunction<unknown, AvatarProps> = (props, ref) => {
  ...
  if (src && isImgExist) {
    childrenToRender = (
      <StyledAvatarImage src={src} draggable={draggable} srcSet={srcSet} onError={handleImgLoadError} alt={alt} blur={blur} />
    );
  } else if (icon) {
    childrenToRender = <StyledAvatarIconContainer>{icon}</StyledAvatarIconContainer>;
